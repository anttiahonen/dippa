...
\label{section:demographics}
    ...
    participants, all can be categorized as senior software developers with many years of working with \textit{xUnit family} testing frameworks.
...
Project A has two developers, \textbf{participants A} and \textbf{B}, working with \textit{Spring Framework} backend,
where there is automated low-level testing in place with \textit{JUnit}.
Project B has only one backend developer, \textbf{participant C}, working with \textit{Spring Framework} . As Project B
...
\section{Surveys and BDD-testing framework feedback interviews analyzed}
...
in them took extra time compared to \textit{JUnit}. Participant B answered to use slightly more time with \textit{Spectrum} on all aspects of low-level testing.
...
participant C. This was also the most important target amongst original study survey participants.
...
slightly more important target than before. These two targets and their higher importance seem natural for a \textit{xSpec family}
...
for already granular unit test cases in project B. This might act as an indicator of already behaviorally described test methods.
...
than with \textit{xUnit family} testing.
...
Q6' studied the change of test output informativeness after the introduction of new BDD-testing framework. Participant A
...
C found it more informative. As the \textit{xSpec family} testing frameworks allow free text description to their
...
\textbf{Test commenting} and \textbf{comment updating} was found beneficial, but also a practice rarely done by most of the developers~\cite{li2016automatically}.
...
This is also in contradiction with the test code analysis \textit{COC}-metric change in project A. Participant C said to add and
update the comments with \textit{Spock} the same amount as before. Test code analysis supports this.
...
Changes in unit testing granularity, after the introduction of BDD-testing framework, were studied with Q10'. Participant A
...
feel maintaining at least somewhat difficult. Runeson also discovered this same difficulty amongst survey participants~\cite{runeson2006survey}.
One of the study hypothesis was that BDD-testing frameworks should
...
been used individually in two different studies~\cite{williams2009effectiveness,li2016automatically}. The original
...
promote the test cases to act better as documentation for the tested production code. Documentation aspect of unit tests was identified in earlier as potential benefit
...
project A showed to be an exception earlier with \textit{JUnit}, as there was approximately at least one comment per test method.
...
seem to promote much better information on test method names than \textit{JUnit}. In project B, \textit{Spock} displayed a 50\% increase in TMNWC.
Tester habbits might be one of the reasons, but also the single line text description for test method naming could be a
reason for a milder increase in words than observed \textit{Spectrum}-tests.
...
usage through the language features, but the framework doesn't support this with a DSL. Earlier with \textit{JUnit}, Project B had about 2\% of
...
\textit{TMNWC} changed in both projects. Project A had almost 2.5 times increased test method name word count with \textit{Spectrum}. In
